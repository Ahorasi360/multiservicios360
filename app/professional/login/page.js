// professional/login/page - BILINGUAL ES/EN
'use client';
...
const T = {
  es: {
    title: 'Portal Profesional', subtitle: 'Multi Servicios 360',
  ...
  en: {
    title: 'Professional Portal', subtitle: 'Multi Servicios 360',
...
export default function ProfessionalLoginPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (data.success && data.professional) {
        localStorage.setItem('profId', data.professional.id);
        localStorage.setItem('profName', data.professional.name);
        localStorage.setItem('profProfession', data.professional.profession);
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#0F172A,#7C3AED,#0F172A)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'#fff', borderRadius:16, padding:'40px 36px', maxWidth:400, width:'100%', boxShadow:'0 25px 50px rgba(0,0,0,0.3)', textAlign:'center' }}>
        ...
        <div style={{ width:56, height:56, background:'linear-gradient(135deg,#7C3AED,#A78BFA)', borderRadius:14, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', fontSize:22 }}>⚖️</div>
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" disabled={loading} style={{ width:'100%', padding:'12px', background:'linear-gradient(135deg,#7C3AED,#A78BFA)', color:'#fff', border:'none', borderRadius:10, fontSize:15, fontWeight:600, cursor:'pointer', opacity:loading?0.6:1 }}>
