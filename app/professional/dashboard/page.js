// app/professional/dashboard/page.js - BILINGUAL ES/EN
'use client';
...
const T = {
  es: {
    ...
    filterAll: 'Todos', loading: 'Cargando casos...', noCases: 'Aún no hay casos asignados. Tu administrador asignará casos para revisión.',
    ...
    statusLabels: { pending: 'Pendiente de Revisión', in_review: 'En Revisión', approved: 'Aprobado', needs_changes: 'Necesita Cambios', completed: 'Completado' },
    serviceLabels: { 
      general_poa: 'Poder Notarial General', limited_poa: 'Poder Notarial Limitado', 
      living_trust: 'Fideicomiso en Vida', llc_formation: 'Formación de LLC',
      bill_of_sale: 'Carta de Venta', affidavit: 'Declaración Jurada',
      authorization_letter: 'Carta de Viaje', travel_authorization: 'Carta de Viaje',
      guardianship_designation: 'Designación de Guardián', revocation_poa: 'Revocación de Poder',
      promissory_note: 'Pagaré', pour_over_will: 'Testamento', simple_will: 'Testamento Simple',
      hipaa_authorization: 'Autorización HIPAA', certification_of_trust: 'Certificación de Fideicomiso',
      ...
      small_estate_affidavit: 'Declaración Patrimonio Pequeño', quitclaim_deed: 'Escritura Quitclaim',
    ...
    profLabels: { attorney: '⚖️ Abogado', notary: '📝 Notario', cpa: '📊 Contador', realtor: '🏠 Agente Inmobiliario', other: '👤 Profesional' },
    doc: 'doc', docs: 'docs',
  },
  en: {
    brand: 'Professional Portal', password: '🔑 Password', signout: 'Sign Out',
    totalCases: 'Total Cases', pendingReview: 'Pending Review', inReview: 'In Review', approved: 'Approved',
    filterAll: 'All', loading: 'Loading cases...', noCases: 'No cases assigned yet. Your administrator will assign cases for review.',
    ...
    startReview: '🔍 Start Review', approveLbl: '✅ Approve', needsChanges: '⚠️ Needs Changes', markCompleted: '🏁 Mark Completed', saveNotes: '💾 Save Notes',
    lastReviewed: 'Last reviewed:', assigned: 'Assigned',
    pwTitle: '🔑 Change Password', pwCurrent: 'Current Password', pwNew: 'New Password', pwConfirm: 'Confirm New Password',
    cancel: 'Cancel', save: 'Update Password', saving: 'Saving...',
    pwMismatch: '❌ Passwords do not match',
    pwOk: '✅ Password updated successfully',
    statusLabels: { pending: 'Pending Review', in_review: 'In Review', approved: 'Approved', needs_changes: 'Needs Changes', completed: 'Completed' },
    serviceLabels: { 
      general_poa: 'General POA', limited_poa: 'Limited POA', 
      living_trust: 'Living Trust', llc_formation: 'LLC Formation',
      bill_of_sale: 'Bill of Sale', affidavit: 'Affidavit',
      authorization_letter: 'Travel Authorization', travel_authorization: 'Travel Authorization',
      guardianship_designation: 'Guardianship Designation', revocation_poa: 'POA Revocation',
      promissory_note: 'Promissory Note', pour_over_will: 'Pour-Over Will', simple_will: 'Simple Will',
      hipaa_authorization: 'HIPAA Authorization', certification_of_trust: 'Certification of Trust',
    ...
    profLabels: { attorney: '⚖️ Attorney', notary: '📝 Notary', cpa: '📊 CPA', realtor: '🏠 Realtor', other: '👤 Professional' },
    doc: 'doc', docs: 'docs',
  }
};
...
export default function ProfessionalDashboard() {
  ...
  useEffect(() => {
    const savedLang = localStorage.getItem('prof_lang') || 'es';
    setLang(savedLang);
  ...
  async function updateCaseStatus(assignmentId, newStatus, notes) {
    try {
      ...
      const data = await res.json();
      if (data.success) { setMessage(`✅ ${newStatus}`); fetchCases(profId); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F1F5F9' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: 20 }}>
        ...
        {/* Cases */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: 40, color: '#64748B' }}>{t.loading}</div>
        ) : cases.length === 0 ? (
                  ...
                  <div onClick={() => { setExpandedCase(isExpanded ? null : c.id); setReviewNotes(c.notes || ''); }}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: '#64748B' }}>
                        {c.client_email || ''} • {t.serviceLabels[c.matter_type] || c.matter_type} • {t.assigned} {fmt(c.assigned_at)}
                      </div>
                    </div>
                      ...
                      <div style={{ paddingTop: 16 }}>
                        {c.vault_token && (
                          <div style={{ background: '#F8FAFC', borderRadius: 8, padding: '12px 16px', marginBottom: 12 }}>
                            <span style={{ fontSize: 12, fontWeight: 600, color: '#64748B' }}>{t.clientVault}</span>
                        ...
                        <div style={{ marginBottom: 14 }}>
                          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 6 }}>{t.reviewNotes}</label>
                          <textarea value={reviewNotes} onChange={e => setReviewNotes(e.target.value)} rows={3} placeholder={t.reviewPlaceholder}
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {c.status === 'pending' && (
                            <button onClick={() => updateCaseStatus(c.id, 'in_review', reviewNotes)} style={actionBtn('#EFF6FF', '#1E3A8A', '#BFDBFE')}>{t.startReview}</button>
                          )}
                          {(c.status === 'pending' || c.status === 'in_review' || c.status === 'needs_changes') && (
                            <>
                              <button onClick={() => updateCaseStatus(c.id, 'approved', reviewNotes)} style={actionBtn('#DCFCE7', '#166534', '#BBF7D0')}>{t.approveLbl}</button>
                              <button onClick={() => updateCaseStatus(c.id, 'needs_changes', reviewNotes)} style={actionBtn('#FEE2E2', '#991B1B', '#FECACA')}>{t.needsChanges}</button>
                            </>
                          )}
                          {c.status === 'approved' && (
                            <button onClick={() => updateCaseStatus(c.id, 'completed', reviewNotes)} style={actionBtn('#F0FDF4', '#059669', '#BBF7D0')}>{t.markCompleted}</button>
                          )}
                          {reviewNotes !== (c.notes || '') && (
                            <button onClick={() => updateCaseStatus(c.id, c.status, reviewNotes)} style={actionBtn('#F8FAFC', '#475569', '#E2E8F0')}>{t.saveNotes}</button>
                          )}
                        </div>
                        {c.reviewed_at && (
                          <div style={{ marginTop: 12, fontSize: 12, color: '#94A3B8' }}>{t.lastReviewed} {fmt(c.reviewed_at)}</div>
                        )}
                      </div>
                    </div>
                  )}
            ...
            <form onSubmit={handleChangePassword}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
                ...
                <button type="submit" disabled={pwSaving}
                  style={{ flex: 1, padding: 12, background: 'linear-gradient(135deg,#7C3AED,#A78BFA)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', opacity: pwSaving ? 0.6 : 1 }}>
