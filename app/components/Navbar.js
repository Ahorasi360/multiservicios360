...
// Slug mapping for pages where EN/ES have different URL slugs
const SLUG_MAP_ES_TO_EN = {
  '/nuestra-historia': '/en/our-story',
  '/por-que-nosotros': '/en/why-us',
  '/contacto': '/en/contact',
  '/mas-servicios': '/en/more-services',
  '/planificacion-familiar': '/en/family-planning',
  '/negocios': '/en/business',
  '/guias': '/en/guias',
  '/blog': '/en/blog',
};
// Build reverse map
const SLUG_MAP_EN_TO_ES = Object.fromEntries(
...
export default function Navbar({ lang = 'es', currentPath = '', langSwitchUrl = null }) {
  ...
  const isEs = lang === 'es';
  // Use auto-detected pathname if currentPath not explicitly provided
  const activePath = currentPath || pathname || '';

  // Compute the alternate language URL
  let altLangUrl;
  if (langSwitchUrl) {
    altLangUrl = langSwitchUrl;
  } else if (isEs) {
    // ES → EN: check slug map first, then prepend /en/
    altLangUrl = SLUG_MAP_ES_TO_EN[activePath]
      || (activePath ? '/en' + activePath : '/en');
  } else {
    // EN → ES: check slug map first, then strip /en prefix
    altLangUrl = SLUG_MAP_EN_TO_ES[activePath]
      || (activePath ? activePath.replace(/^\/en/, '') || '/' : '/');
  }
