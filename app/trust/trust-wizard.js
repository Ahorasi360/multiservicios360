          ...
          <div style={st.grid2}>
            <div style={st.card}>
              ...
              {TIERS.map(tier => (
                <button key={tier.value} onClick={() => setReviewTier(tier.value)} style={{ ...st.tierCard, ...(reviewTier === tier.value ? st.tierCardSelected : {}) }}>
                  ...
                  {tier.attorney_note_en && <div style={{ marginTop: '8px', padding: '8px', backgroundColor: '#FEF3C7', borderRadius: '6px', fontSize: '11px', color: '#92400E' }}>{language === 'en' ? tier.attorney_note_en : tier.attorney_note_es}</div>}
                </button>
              ))}
            ...
            <div>
              <div style={{ ...st.card, marginBottom: '16px' }}>
                ...
                <div><button onClick={handleSubmit} disabled={isLoading} style={{ ...st.btnPrimary, marginTop: '16px', ...(isLoading ? st.btnDisabled : st.btnPurple) }}>{isLoading ? t.paying : `${t.submit} - $${calculateTotal(reviewTier, selectedUpsells)}`}</button><p style={{ fontSize: '11px', color: '#6B7280', textAlign: 'center', marginTop: '8px' }}>{language === 'en' ? 'Payment is for platform access only. This is not a payment for legal services.' : 'El pago es solo por acceso a la plataforma. Este no es un pago por servicios legales.'}</p></div>
              </div>
