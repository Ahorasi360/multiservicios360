...
const translations = {
  es: {
    heroTitle: 'Blog y Recursos',
    heroSub: 'Artículos legales, noticias y recursos para la comunidad latina en California',
    readMore: 'Leer más →',
    noPosts: 'No hay artículos en esta categoría todavía.',
    minRead: 'min de lectura',
    needDocs: '¿Necesita preparar documentos legales?',
    platform: 'Nuestra plataforma le permite hacerlo usted mismo, en español e inglés, en minutos.',
    ...
    platformDesc: 'Plataforma de preparación de documentos legales de autoayuda para la comunidad latina en California.',
    terms: 'Términos',
    privacy: 'Privacidad',
    ...
    llcBtn: 'Formar LLC →',
    llcService: 'Formación de LLC',
    hours: 'Lun - Vie: 9am - 6pm',
  },
  en: {
    heroTitle: 'Blog & Resources',
    heroSub: 'Legal articles, news, and resources for the Latino community in California',
    readMore: 'Read more →',
    noPosts: 'No posts in this category yet.',
    minRead: 'min read',
    needDocs: 'Need to prepare legal documents?',
    platform: 'Our platform lets you do it yourself, in Spanish and English, in minutes.',
    ...
    platformDesc: 'Self-help legal document preparation platform for the Latino community in California.',
    terms: 'Terms',
    privacy: 'Privacy',
    poaBtn: 'General POA →',
    lpoaBtn: 'Limited POA →',
...
export default function BlogListClient({ posts, lang }) {
  lang = lang || 'es';
  ...
  const toggleUrl = lang === 'es' ? '/en/blog' : '/blog';
    ...
    <div className="min-h-screen bg-white text-gray-900">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((post) => {
                ...
                const postUrl = blogBase + '/' + post.slug;
                return (
      ...
      <section className="bg-white py-12 border-t border-gray-100">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-xl sm:text-2xl font-extrabold text-gray-900 mb-3">{t.needDocs}</h2>
          <p className="text-gray-500 text-sm mb-6">{t.platform}</p>
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link href="/limited-poa" className="bg-teal-600 hover:bg-teal-700 text-white font-bold text-sm px-5 py-3 rounded-lg no-underline shadow-md transition-colors">{t.lpoaBtn}</Link>
            ...
            <Link href="/llc" className="bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm px-5 py-3 rounded-lg no-underline shadow-md transition-colors">{t.llcBtn}</Link>
            <Link href="/mas-servicios" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm px-5 py-3 rounded-lg no-underline shadow-md transition-colors">{lang === 'es' ? 'Más Servicios →' : 'More Services →'}</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-gray-900 text-white pt-10 pb-20 md:pb-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid sm:grid-cols-3 gap-8 mb-6">
            <div><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 bg-blue-700 rounded-lg flex items-center justify-center"><span className="text-white font-extrabold text-[10px]">M360</span></div><span className="text-white font-bold text-sm">Multi Servicios 360</span></div><p className="text-gray-400 text-xs leading-relaxed">{t.platformDesc}</p></div>
            <div><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2">{t.services}</h3><ul className="space-y-1 list-none p-0 m-0"><li><Link href="/poa" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Poder Notarial General' : 'General Power of Attorney'}</Link></li><li><Link href="/limited-poa" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Poder Notarial Limitado' : 'Limited Power of Attorney'}</Link></li><li><Link href="/trust" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Fideicomiso en Vida' : 'California Living Trust'}</Link></li><li><Link href="/llc" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Formación de LLC' : 'LLC Formation'}</Link></li><li><Link href="/travel-authorization" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Carta de Autorización de Viaje' : 'Travel Authorization'}</Link></li><li><Link href="/bill-of-sale" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Carta de Venta' : 'Bill of Sale'}</Link></li><li><Link href="/affidavit" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Declaración Jurada' : 'Affidavit'}</Link></li><li><Link href="/promissory-note" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Pagaré' : 'Promissory Note'}</Link></li><li><Link href="/guardianship" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Designación de Guardián' : 'Guardianship Designation'}</Link></li><li><Link href="/revocation-poa" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Revocación de Poder' : 'POA Revocation'}</Link></li><li><Link href="/simple-will" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Testamento Simple' : 'Simple Will'}</Link></li><li><Link href="/pour-over-will" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Testamento de Traspaso' : 'Pour-Over Will'}</Link></li><li><Link href="/hipaa-authorization" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Autorización HIPAA' : 'HIPAA Authorization'}</Link></li><li><Link href="/s-corp-formation" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Formación S-Corporation' : 'S-Corp Formation'}</Link></li><li><Link href="/c-corp-formation" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Formación C-Corporation' : 'C-Corp Formation'}</Link></li><li><Link href="/corporate-minutes" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Actas Corporativas' : 'Corporate Minutes'}</Link></li><li><Link href="/banking-resolution" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Resolución Bancaria' : 'Banking Resolution'}</Link></li><li><Link href="/small-estate-affidavit" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Declaración Jurada de Sucesión' : 'Small Estate Affidavit'}</Link></li><li><Link href="/quitclaim-deed" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Escritura de Traspaso' : 'Quitclaim Deed'}</Link></li><li><Link href="/contractor-agreement" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Contrato de Contratista' : 'Contractor Agreement'}</Link></li><li><Link href="/demand-letter" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Carta de Demanda' : 'Demand Letter'}</Link></li><li><Link href="/apostille-letter" className="text-gray-400 hover:text-white text-xs no-underline">{lang === 'es' ? 'Solicitud de Apostilla' : 'Apostille Request'}</Link></li></ul></div>
            <div><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2">{t.contact}</h3><ul className="space-y-1 list-none p-0 m-0"><li><a href="tel:8552467274" className="text-gray-400 hover:text-white text-xs no-underline">855.246.7274</a></li><li><a href="mailto:info@multiservicios360.net" className="text-gray-400 hover:text-white text-xs no-underline">info@multiservicios360.net</a></li><li className="text-gray-500 text-xs">{t.hours}</li><li className="text-gray-500 text-xs">Beverly Hills, CA</li></ul></div>
          </div>
          <div className="border-t border-gray-800 pt-4">
            <div className="flex flex-wrap items-center justify-center gap-4 mb-3"><Link href="/nuestra-historia" className="text-gray-500 hover:text-white text-xs no-underline">{t.ourStory}</Link><Link href="/contacto" className="text-gray-500 hover:text-white text-xs no-underline">{t.contact}</Link><Link href="/terms" className="text-gray-500 hover:text-white text-xs no-underline">{t.terms}</Link><Link href="/privacy" className="text-gray-500 hover:text-white text-xs no-underline">{t.privacy}</Link><Link href={blogBase} className="text-gray-500 hover:text-white text-xs no-underline">Blog</Link></div>
