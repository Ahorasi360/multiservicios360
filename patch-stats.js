...
export async function GET(request) {
  try {
    ...
    // Count documents from all 4 matter tables
    const { count: poaCount } = await supabase
      .from('poa_matters')
      .select('*', { count: 'exact', head: true })
      .eq('partner_id', partnerId);

    const { count: limitedPoaCount } = await supabase
    ...
    const { count: llcCount } = await supabase
      .from('llc_matters')
      .select('*', { count: 'exact', head: true })
      .eq('partner_id', partnerId);

    const totalDocuments = (poaCount || 0) + (limitedPoaCount || 0) + (trustCount || 0) + (llcCount || 0);

    // Get referrals/commissions from partner_referrals table
