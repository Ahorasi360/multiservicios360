...
export async function sendWelcomeEmail({ to, name, role, loginUrl, email, password, setupFee, membershipUrl, tier, commissionRate, alreadyPaid }) {
  const resend = new Resend(process.env.RESEND_API_KEY);
  const roleLabels = {
    staff: 'Staff Member',
    partner: 'Partner',
    sales: 'Sales Representative',
    professional: 'Attorney / Notary',
  };
  const roleLabelsEs = {
    staff: 'Miembro del Equipo',
    partner: 'Socio',
    sales: 'Representante de Ventas',
    professional: 'Abogado / Notario',
  };
  const roleColors = {
    staff: '#059669',
    partner: '#2563EB',
...
<body style="margin:0;padding:0;background:#F1F5F9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:520px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="padding:28px;">
      ...
      ${alreadyPaid && tier ? `
      <div style="background:#F0FDF4;border:2px solid #22C55E;border-radius:12px;padding:16px 20px;margin:0 0 20px;">
        <p style="color:#166534;font-size:13px;font-weight:700;margin:0 0 10px;">✅ Su plan activo / Your active plan:</p>
        <p style="color:#0F172A;font-size:20px;font-weight:800;margin:0 0 4px;">${tier}</p>
        <p style="color:#15803D;font-size:15px;font-weight:700;margin:0 0 6px;">${commissionRate}% comisión por cada venta</p>
        <p style="color:#166534;font-size:12px;margin:0;">Membresía activa por 1 año ✓</p>
      </div>
      ` : setupFee ? `
