...
export async function notifyOwnerOfSale(sale) {
  const { documentType, clientName, clientEmail, amount, matterId, partnerName } = sale;
  ...
  const adminEmail = process.env.ADMIN_NOTIFY_EMAIL || 'agaleano@multiservicios360.net';
  const adminPhone = process.env.ADMIN_NOTIFY_PHONE; // e.g. +16195551234

  const subject = isManual
    ? `🔴 ACTION REQUIRED: LLC Sale — ${clientName} — ${amountStr}`
    : `✅ New Sale: ${docLabel.split('—')[0].trim()} — ${amountStr}`;
